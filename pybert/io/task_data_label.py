...
class TaskData(object):
    ...
    def train_val_split(self,X, y,valid_size,stratify=False,shuffle=True,save = True,
        ...
        """
            Spit the raw dataset into training and validation

            X: Dataset without labels
            y: Labels associated with datapoints in X
        ...
        if save:
            ...
            train_path = data_dir / f"{data_name}.label_train.pkl"
            valid_path = data_dir / f"{data_name}.label_valid.pkl"
    ...
    def save_test_data(self,X, y, save = True, seed = None,data_name = None,data_dir = None):

        pbar = ProgressBar(n_total=len(X))
        logger.info('saving the test data in a pickle')
        ...
        if save:
            ...
            test_path = data_dir / f"{data_name}.label_test.pkl"
            print ("Test data:")
            print (data)
            # valid_path = data_dir / f"{data_name}.valid.pkl"
            save_pickle(data=data,file_path=test_path)
            # save_pickle(data = valid,file_path=valid_path)
